...
import numpy as np
import argparse
from DQN import DQN  # Import from your standalone file

def generate_dummy_states(batch_size=32, height=168, width=168, channels=4):
    """Generate random grayscale frames for testing."""
...
def analyze_q_values(model_path):
    ...
    # Initialize DQN with matching parameters from server
    model = DQN(input_shape=(4, 168, 168), num_actions=4).to(device)
    state_dict = torch.load(model_path, map_location=device)
    model.load_state_dict(state_dict)
    model.eval()  # Set to evaluation mode
    
    # Create a target network (identical for this analysis)
    target_net = DQN(input_shape=(4, 168, 168), num_actions=4).to(device)
    target_net.load_state_dict(state_dict)
    target_net.eval()
    
    # Generate dummy states for analysis
    batch_size = 32
    states = generate_dummy_states(batch_size).to(device)
    ...
    action_consistency = 1.0  # Full exploitation in eval mode
