...
def detect_ready(screen):
        """Detect 'READY!' using template matching at specific position."""
        # Load the template (adjust path as needed)
        template = cv2.imread("sample_ready.jpg", cv2.IMREAD_GRAYSCALE)
        if template is None:
            print("Error: Could not load ready.jpg")
            return False
...
def test_detect_ready(image_path="ready.png"):
    ...
    # Test the detection
    is_game_over = detect_ready(screen)


if __name__ == "__main__":
    # Specify your test image path here
    test_image_path = "ready.jpg"
    test_detect_ready(test_image_path)